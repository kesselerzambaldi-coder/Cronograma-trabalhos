...
export default function App() {
  ...
  return (
    <div className="min-h-screen pb-20 bg-slate-50">
      {/* Header */}
      <header className="bg-white border-b border-brand-line px-6 py-10 flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="group relative">
          ...
          <div className="flex items-start gap-4">
            ...
            <button 
              ...
              className="p-2 hover:bg-slate-100 rounded-full transition-colors mt-2"
              title="Editar Título"
            >
        ...
        <div className="flex flex-col items-end gap-6">
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-700 text-[11px] font-bold uppercase transition-all rounded-lg hover:bg-blue-600 hover:text-white cursor-pointer">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-700 text-[11px] font-bold uppercase transition-all rounded-lg hover:bg-blue-600 hover:text-white"
            >
            ...
            <button 
              onClick={exportToPDF}
              className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-[11px] font-bold uppercase shadow-lg shadow-blue-200 transition-all rounded-lg hover:bg-blue-700 hover:-translate-y-0.5"
            >
          ...
          <div className="text-right">
            <div className="flex items-center gap-2 text-2xl font-mono font-bold justify-end text-slate-800">
              <Clock className="w-6 h-6 text-blue-600" />
              <span>{stats.daysLeft} DIAS</span>
            </div>
            <div className="text-[10px] font-mono uppercase tracking-widest text-slate-400 font-medium">
              META: {format(parseISO(data.deadline), 'dd MMMM yyyy', { locale: ptBR })}
            </div>
          </div>
        </div>
      </header>
      ...
      {/* Stats Grid */}
      <section className="grid grid-cols-1 md:grid-cols-3 border-b border-brand-line bg-white shadow-sm mx-6 mt-[-2rem] rounded-2xl overflow-hidden z-10 relative">
        <div className="p-8 md:border-r border-brand-line">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-6xl font-black font-mono tracking-tighter text-blue-600">{stats.avgProgress.toFixed(1)}</span>
        ...
        <div className="p-8 md:border-r border-brand-line">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-6xl font-black font-mono tracking-tighter text-slate-900">{stats.completed}</span>
            <span className="text-2xl font-bold text-slate-300">/ {stats.total}</span>
          </div>
          <p className="mt-4 text-[10px] font-mono text-slate-400 uppercase tracking-widest">Atividades em campo: {stats.total - stats.completed}</p>
        </div>

        <div className="p-8">
          <div className="text-[10px] font-mono uppercase tracking-widest text-slate-400 mb-4 font-bold">03. STATUS CRONOGRAMA</div>
          <div className="flex flex-col gap-4">
            {stats.daysLeft < 15 ? (
              <div className="bg-red-50 text-red-600 px-4 py-3 rounded-xl flex items-center gap-3 font-bold border border-red-100">
                ...
                <span className="text-sm">ALERTA CRÍTICO</span>
              </div>
            ) : (
              <div className="bg-emerald-50 text-emerald-700 px-4 py-3 rounded-xl flex items-center gap-3 font-bold border border-emerald-100">
                ...
                <span className="text-sm">NO PRAZO / EM DIA</span>
              </div>
            )}
            <div className="flex flex-wrap gap-2">
              <span className="px-2 py-1 bg-slate-900 text-white text-[9px] font-mono rounded">VERSÃO v2.0</span>
              <span className="px-2 py-1 bg-white border border-slate-200 text-[9px] font-mono text-slate-500 rounded">SINCRO: {format(new Date(), 'HH:mm')}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Controls */}
      <div className="sticky top-0 bg-slate-50/80 backdrop-blur-xl z-20 border-b border-slate-200 px-6 py-6 flex flex-col md:flex-row gap-6 items-center justify-between mt-4">
        <div className="flex items-center gap-3 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-hide">
          <button 
            onClick={() => setActiveSectionId('all')}
            className={cn(
              "whitespace-nowrap px-5 py-2 rounded-xl text-[10px] font-bold uppercase transition-all shadow-sm border",
              activeSectionId === 'all' 
        ...
        <div className="relative w-full md:w-80 shadow-sm rounded-xl overflow-hidden border border-slate-200 bg-white">
          ...
          <input 
            ...
            className="w-full bg-transparent px-12 py-3 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all text-slate-700"
          />
        </div>
      </div>
          ...
          {filteredActivities.length === 0 && (
            <div className="py-32 text-center bg-slate-50/50">
              <p className="font-mono text-xs text-slate-400 font-bold uppercase tracking-widest bg-white px-6 py-3 border border-slate-200 inline-block rounded-2xl shadow-sm italic">
                Nenhuma atividade localizada no filtro atual
              </p>
            </div>
          )}
        </div>
      ...
      <motion.button 
        ...
        className="fixed bottom-10 right-10 w-16 h-16 bg-blue-600 text-white rounded-2xl shadow-2xl shadow-blue-300 flex items-center justify-center hover:bg-blue-700 transition-all z-40 border-4 border-white"
      >
      ...
      <AnimatePresence>
        {isSettingsOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white w-full max-w-lg border border-slate-200 shadow-2xl rounded-3xl p-8 max-h-[90vh] overflow-y-auto scrollbar-hide">
               <div className="flex items-center justify-between mb-8">
                 <h2 className="text-sm font-bold uppercase tracking-widest text-slate-400 font-mono">Painel do Projeto</h2>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white w-full max-w-xl overflow-hidden border border-slate-200 shadow-2xl rounded-3xl">
              <div className="px-8 py-6 flex items-center justify-between border-b border-slate-100 bg-slate-50/50">
                <h2 className="text-xs font-bold uppercase font-mono tracking-widest text-slate-800">
                  {editingId ? 'Editar Registro' : 'Nova Atividade'}
                </h2>
                <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-slate-200 rounded-full transition-colors text-slate-400">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <form onSubmit={handleSaveTask} className="p-8 flex flex-col gap-6">
                <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                  <div className="col-span-2">
                    <label className="text-[10px] font-bold uppercase text-slate-400 mb-2 block">Nome da Atividade</label>
                    <input required type="text" value={formTask.name} onChange={(e) => setFormTask({ ...formTask, name: e.target.value })} className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl text-sm font-bold text-slate-900 focus:ring-2 focus:ring-blue-100 outline-none" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold uppercase text-slate-400 mb-2 block">Sessão / Módulo</label>
                    <select value={formTask.section} onChange={(e) => setFormTask({ ...formTask, section: e.target.value })} className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl text-sm font-bold text-slate-700 outline-none">
                      {data.sections.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold uppercase text-slate-400 mb-2 block">Célula / Equipe</label>
                    <input type="text" value={formTask.who} onChange={(e) => setFormTask({ ...formTask, who: e.target.value })} className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl text-sm font-bold text-slate-700 outline-none" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold uppercase text-slate-400 mb-2 block">Materiais Necessários</label>
                    <input type="text" value={formTask.material} onChange={(e) => setFormTask({ ...formTask, material: e.target.value })} className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl text-sm font-bold text-slate-700 outline-none" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold uppercase text-slate-400 mb-2 block">Pendências Impeditivas</label>
                    <input type="text" placeholder="Escreva 'ok' se não houver" value={formTask.pending} onChange={(e) => setFormTask({ ...formTask, pending: e.target.value })} className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl text-sm font-bold text-slate-700 outline-none" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold uppercase text-slate-400 mb-2 block">Estimativa HH (Dias)</label>
                    <input type="number" step="0.5" value={formTask.duration} onChange={(e) => setFormTask({ ...formTask, duration: parseFloat(e.target.value) })} className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl text-sm font-bold text-slate-700 outline-none" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold uppercase text-slate-400 mb-2 block">Progresso Atual (%)</label>
                    <input type="number" min="0" max="100" value={formTask.progress} onChange={(e) => setFormTask({ ...formTask, progress: parseInt(e.target.value) })} className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl text-sm font-bold text-slate-700 outline-none" />
                  </div>
                </div>
                <button type="submit" className="mt-4 w-full bg-blue-600 text-white font-bold uppercase text-xs py-4 rounded-2xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 tracking-widest">
                  {editingId ? 'Confirmar Edição' : 'Registrar Atividade'}
                </button>
              </form>
            </motion.div>
          </div>
